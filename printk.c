...
size_t bprintu32(char* buf, uint32_t a, int base)
{
    size_t i;
    size_t bytes;
...
size_t bprints32(char* buf, int32_t a, int base)
{
    if (a < 0) {
        *(buf++) = '-';
...
size_t bprintptr(char* buf, void* ptr)
{
    ...
    return bytes + bprintu32(buf + bytes, (uint32_t) ptr, 16);
}
